...
class rettyScraping():
  ...
  for i in range(len(pre)):
      if i <= 8:
          channel[pre[i]] = str(0)+str(i+1)
      else:
          channel[pre[i]] = str(i+1)
  ...
  def getData(self,where,Min,Max,csv):
   ...
   def getRestaurantData(self,goto,df):
      ...
      data["phone"] = call
      data["url"] = Url
      df = getComment(goto,k_num,data,df)
      return df
  ...
  def getComment(self,goto,size,data,df):
      for i in range(-(-int(size)//20)):
          ...
          for dictionary in En:
              ...
              dictionary = re.sub("\"tagIds\".*,\"scoreTypeString\"","\"scoreTypeString\"",kkk)
              try:
