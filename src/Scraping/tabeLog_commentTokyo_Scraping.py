...
class tabeLog_comment():
    ...
    def getOneComment(self, url):
        try:
            ...
            comment = soup.find_all(
                "div", class_=re.compile("rvw-item__rvw-comment"))[0]
    ...
    def getComment(self, name):
        ...
        comment_csv_path = "T_comment_"+name+".csv"
        ...
        if os.path.isfile(comment_csv_path):
            df = pandas.read_csv(comment_csv_path)
        ...
        with open(csv_path) as f:
            ...
            for row in reader:
                ...
                for i in range(-(-int(size)//20)):
                    Ht = requests.get(
                        row[1] + "dtlrvwlst/COND-0/smp1/?smp=1&lc=0&rvw_part=all&PG=" + str(i+1) + "/")
                    soup = BeautifulSoup(Ht.content, 'lxml')

                    elems_n = soup.find_all("div", class_=re.compile(
                        "rvw-item js-rvw-item-clickable-area"))

                    for link in elems_n:
                        tourl = "https://tabelog.com" + \
                            link.get("data-detail-url")

                        if tourl not in read_urls:
                            all_score = link.find_all("b", class_=re.compile(
                                "c-rating__val c-rating__val--strong"))
                            try:
                                data["評価"] = all_score[0].text
                            except Exception:
                                data["評価"] = "-1"
                            userId = link.find_all(
                                "a", target=re.compile("_blank"))[0]
                            try:
                            ...
                            kkk = link.find_all("strong", class_=re.compile(
                                "rvw-item__ratings-dtlscore-score"))
                ...
                try:
                    ...
                    continue
                if not os.path.isfile(comment_csv_path):
                    df.to_csv(comment_csv_path)
                    flag = False
                else:
                    df.to_csv(comment_csv_path, mode='a', header=False)
                time.sleep(1)
        bar.finish()
