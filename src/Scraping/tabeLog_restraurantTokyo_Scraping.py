...
class tabeLog_restraurant():
    ...
    def getData(self,name):
        ...
        city_links = soup.find_all("a",class_=re.compile("c-link-arrow"))
        for ii in range(len(city_links)):
            city_name = city_links[ii].find('span').text
            url = city_links[ii].get("href")

            print("{0}/{1} {2}".format(ii+1, len(city_links), city_name))
    ...
    def getRestaurantData(self,goto,df,df2,where):
        ...
        review_link_badge = review_link.find(class_ = "rstdtl-navi__total-count")
